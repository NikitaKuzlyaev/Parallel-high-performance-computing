...
class Parser:
    # Разделы, после которых парсинг прекращается
    STOP_SECTIONS = {
        "see also",
        "notes",
        "references",
    ...
    def parse(self, html: str, page_path: str) -> PageParseResult:
        # Создание DOM-дерева страницы
        soup = BeautifulSoup(html, "html.parser")

        article_root = self._get_article_root(soup)  # Основной блок статьи
        province = self._get_page_title(soup)  # Название страницы (провинции)

        # Если не удалось найти содержимое статьи — возвращаем ошибку
        ...
        headings = self._get_section_headings(article_root)  # Получение всех секций
        sections: list[SectionStat] = []

        for index, heading in enumerate(headings):
            title = self._extract_heading_text(heading)  # Текст заголовка
            if not title:
                continue

            # Остановка при достижении служебных разделов
            if title.lower() in self.STOP_SECTIONS:
                break

            next_heading = headings[index + 1] if index + 1 < len(headings) else None

            # Подсчёт содержимого секции
    ...
    def _get_article_root(self, soup: BeautifulSoup) -> Tag | None:
        # Поиск основного блока статьи по различным селекторам
        selectors = (
        ...
        # Резервный вариант — поиск по наличию заголовков
    ...
    def _get_section_headings(self, article_root: Tag) -> list[Tag]:
        # Сбор всех заголовков второго уровня (h2)
        headings: list[Tag] = []
        seen: set[int] = set()
    ...
    def _get_page_title(self, soup: BeautifulSoup) -> str:
        ...
        # Альтернативный способ через <title>
    ...
    def _count_section_content(self, start_heading: Tag, end_heading: Tag | None) -> tuple[int, int, int]:
        # Подсчёт уникальных изображений, таблиц и ссылок
        ...
        # Обход DOM до следующего заголовка
    ...
    def _is_inside_heading(self, node: Tag) -> bool:
        # Проверка, находится ли элемент внутри заголовка
    ...
    def _is_non_content_image(self, img: Tag) -> bool:
        # Фильтрация служебных изображений (иконки, логотипы и т.п.)
    ...
    def _is_non_content_table(self, table: Tag) -> bool:
        # Фильтрация служебных таблиц (навигация, сайдбары и т.д.)
