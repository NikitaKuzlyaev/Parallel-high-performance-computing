...
__kernel void embossDownscale2x_2d2d(
    __global const uchar* input,
    ...
    const int outHeight)
{
    ...
    int oy = get_group_id(1) * get_local_size(1) + get_local_id(1);
    ...
    for (int c = 0; c < 3; c++) {
        ...
        int p10 = (int)embossAt(input, width, height, x1, y0, c);
        int p01 = (int)embossAt(input, width, height, x0, y1, c);
        int p11 = (int)embossAt(input, width, height, x1, y1, c);
