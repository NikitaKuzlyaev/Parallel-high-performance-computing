...
public class OpenClFrameProcessor1d1d implements GpuFrameProcessor {
    ...
    public OpenClFrameProcessor1d1d(String kernelSource) throws IOException {
        ...
        // Ядро
        kernel = clCreateKernel(program, "embossDownscale2x_1d1d", null);
    }
    ...
    public synchronized byte[] process(byte[] input, int width, int height) {
        ...
        try {
            ...
            int localSize = 256;
            int globalSize = ((outWidth * outHeight + localSize - 1) / localSize) * localSize;
